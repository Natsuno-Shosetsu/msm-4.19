...
static int sn_fuse_probe(struct platform_device *pdev)
{
	...
	if (!drvdata) {
		pr_err("devm kzalloc for serial number and fuse state failed !!! \n");
		return -ENOMEM;
	}
	...
	if (!res) {
		pr_info("cann't get resource for serial number\n");
		return -ENODEV;
	}

	drvdata->serial_base = devm_ioremap(dev, res->start, resource_size(res));
	if (!drvdata->serial_base) {
		pr_info("cann't do devm io remap for serial number\n");
		return -ENOMEM;
	}

	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "fuse-state");
	if (!res) {
		pr_info("cann't get resource for fuse state\n");
		return -ENODEV;
	}

	drvdata->fuse_base= devm_ioremap(dev, res->start, resource_size(res));
	if (!drvdata->fuse_base) {
		pr_info("cann't do devm io remap for fuse state\n");
		return -ENOMEM;
	}
